...
cd terraform
terraform destroy -var-file=t2large.tfvars -auto-approve && sleep 20 && terraform apply -var-file=t2large.tfvars -auto-approve && terraform output -json | jq 'with_entries(.value |= .value)' > config.json
sleep 60
cd ../grafana
terraform destroy --auto-approve && terraform apply --auto-approve
sleep 60
